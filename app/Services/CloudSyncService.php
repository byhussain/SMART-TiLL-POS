...
class CloudSyncService
{
    ...
    private function mapMorphRelationsToLocalIds(string $table, array $payload, int $localStoreId): array
    {
        ...
        foreach (['transactionable', 'referenceable', 'payable', 'imageable', 'activityable'] as $prefix) {
            ...
            // Canonicalize to the morph ALIAS Eloquent will write for
            // newly created local rows (e.g. 'App\Models\Customer'),
            // NOT the FQCN. Storing the FQCN here breaks any later query
            // that filters by `transactionable_type` against rows freshly
            // created on the POS — TransactionObserver's running-balance
            // lookup misses every bootstrapped row, so credit-sale
            // balances start from 0 instead of including the customer's
            // pre-existing debt.
     ...
     * instantiated. This guarantees the `*_type` string we persist
     * matches what `Model::getMorphClass()` produces for a freshly-created
     * local row — so subsequent `where('transactionable_type', …)`
     * queries (e.g. TransactionObserver's running-balance lookup) hit
     * both the bootstrapped rows and the new ones.
     */
